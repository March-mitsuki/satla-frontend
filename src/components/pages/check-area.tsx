...
const CheckArea: ParentComponent<{
  ...
  const startDragHandler = (
    ...
    onmousemove = (e: MouseEvent) => {
      ...
      if (moveY + (currentFormWrapper as HTMLDivElement).offsetHeight <= docHeight && moveY >= 0) {
        // 开始拖拽
        const deepcopy = (attachedInfo() as AttachedInfo[]).map((x) => x);
        deepcopy[idx].zIndex = 1000;
        deepcopy[idx].position = "absolute";
        (deepcopy[idx].isFloating = true), (deepcopy[idx].y = moveY);
      ...
      if (belowElem instanceof HTMLDivElement && belowElem.closest("form")) {
        afterFormWrapper = belowElem.closest("form")?.parentNode;
        if (afterFormWrapper) {
          const dropElemId = Number(
            (afterFormWrapper as HTMLDivElement).id.replace("-wrapper", ""),
          );
          const reorderIdx = (attachedInfo() as AttachedInfo[]).findIndex(
            (elem) => elem.id === dropElemId,
          );
  ...
  const reorderSubFrontSelf = (idx: number, reorderIdx: number, dragSubtitle: Subtitle) => {
    const dc_attachedInfo = attachedInfo()?.map((x) => x);
    if (!dc_attachedInfo) {
      return;
    }
  ...
  const reorderSubBackSelf = (idx: number, reorderIdx: number, dragSubtitle: Subtitle) => {
    const dc_attachedInfo = attachedInfo()?.map((x) => x);
    if (!dc_attachedInfo) {
      return;
    }
      ...
      switch (data.head.cmd) {
        ...
        case "sAddTransSub": {
          ...
          if (pagetype() === false) {
            ...
            document
              .getElementById(((subtitles() as Subtitle[]).length - 1).toString() + "-sub")
              ?.scrollIntoView();
          }
          break;
        }
                    ...
                    <Switch
                      fallback={
                        <div
                          ...
                          classList={{
                            "cursor-move flex gap-3 w-[180px] items-center px-1 rounded-md bg-orange-500/70 select-none":
                              canOrder() === true,
                            "flex gap-3 w-[180px] items-center px-1 rounded-md bg-orange-500/70 select-none":
                              canOrder() === false,
                          }}
                        >
                    ...
                    >
                      <Match when={(attachedInfo() as AttachedInfo[])[idx()].isEditing}>
                        <div
                          onMouseDown={(e) => startDragHandler(e, idx(), elem)}
                          classList={{
                            "cursor-move flex gap-3 w-[180px] items-center px-1 rounded-md bg-red-500/70 select-none":
                              canOrder() === true,
                            "flex gap-3 w-[180px] items-center px-1 rounded-md bg-red-500/70 select-none":
                              canOrder() === false,
                          }}
                        >
                      ...
                      <Match when={elem.checked_by !== null && elem.checked_by !== ""}>
                        <div
                          onMouseDown={(e) => startDragHandler(e, idx(), elem)}
                          classList={{
                            "cursor-move flex gap-3 w-[180px] items-center px-1 rounded-md bg-green-500/70 select-none":
                              canOrder() === true,
                            "flex gap-3 w-[180px] items-center px-1 rounded-md bg-green-500/70 select-none":
                              canOrder() === false,
                          }}
                        >
                          <div class="flex-1">{elem.input_time}</div>
